...
MATCH (t:THOUGHT)
MATCH (c:CONTENT)
WHERE t.name = "thought.ANTI SEMITISM" AND c.name = "content.ANTI SEMITISM"
MERGE (t)-[:HAS_CONTENT {name: "edge.ANTI SEMITISM"}]->(c);

MATCH (parent:TOPIC)
MATCH (child:THOUGHT)
WHERE parent.name = "topic.PSYCHOLOGY" AND child.name = "thought.ANTI SEMITISM"
MERGE (parent)-[:HAS_THOUGHT {name: "t.edge.PSYCHOLOGY->ANTI SEMITISM"}]->(child);
```
