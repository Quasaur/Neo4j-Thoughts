...
MERGE (t)-[:HAS_CONTENT { "name": "t.edge.DEVIL THE ACCUSER" }]->(c);

MATCH (parent:TOPIC {name: "topic.GUILT"})
MATCH (child:THOUGHT {name: "thought.DEVIL THE ACCUSER"})
MERGE (parent)-[:HAS_THOUGHT { "name": "t.edge.GUILT->DEVIL THE ACCUSER" }]->(child);
```
