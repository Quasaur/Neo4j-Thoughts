#!/usr/bin/env python3
"""
Convert THOUGHTS from static markdown + cypher files to Obsidian-Cypher format.
This script:
1. Maps cypher files to their corresponding markdown files
...
CYPHER_DIR = THOUGHTS_DIR / "cypher" / "done"
...
def find_markdown_file(cypher_name: str) -> Optional[Path]:
    ...
    for subdir in THOUGHTS_DIR.iterdir():
        if subdir.is_dir() and subdir.name in "abcdefghijlmnopqrstuvwy0":
            # Look for exact match (case-sensitive)
            md_file = subdir / f"{thought_name}.md"
            if md_file.exists():
                return md_file
...
def main():
    ...
    if failed:
        print(f"\n⚠️  Failed files are likely in the UnInserted folder")
