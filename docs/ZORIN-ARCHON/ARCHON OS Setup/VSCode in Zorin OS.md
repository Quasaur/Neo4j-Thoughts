# Q:
How do I install VSCode on Zorin OS?
# A:
...
Head to the official VS Code download page and grab the `.deb` file for Debian/Ubuntu-based systems A.
2. Install via terminal
Open a terminal and run:sudo apt install ./<filename>.deb
Replace `<filename>.deb` with the actual file name you downloaded.
3. (Optional) Enable auto-updates
If you want VS Code to update automatically via APT:sudo apt-get install wget gpg
...
Flatpak versions run in a sandbox and may block access to host SDKs, compilers, or shells — which can be a headache for advanced workflows B. If you’ve already installed it via Flatpak and hit limitations, switching to the `.deb` version will give you full control.
